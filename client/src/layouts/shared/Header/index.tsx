      ...
      <ul>
        {tabList.map(tabId => {
          ...
          return (
            <li
              key={page.path}
              className={page.path === location.pathname ? "active" : undefined}
            >
...
export const PublicHeader = () => (
  <header className="polygive-header">
    <h1>
      <Link to={"/"}>Polygive</Link>
    </h1>
    <NavBar />
    ...
    <Dropdown
      contents={
        <>
          ...
          {user.isSuper && (
            <DropdownLink to="/superpanel">Super Panel</DropdownLink>
          )}
          <DropdownExternalLink href={`${apiUrl}/logout`}>
            Log Out
          </DropdownExternalLink>
        </>
      }
